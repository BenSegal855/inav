...
static void crsfFrameAttitude(sbuf_t *dst)
{
    int32_t flags = isArmingDisabledReason();
     ...
     crsfSerialize16(dst, (int16_t)flags & 0xffff);  //Used to be roll sending bottom half
     crsfSerialize16(dst, (int16_t)flags >> 16 );    //Used to be yaw sending top half
}
...
static void crsfFrameFlightMode(sbuf_t *dst)
{
    ...
    sprintf(flightMode, "%d", isArmingDisabledReason()); //Take this out when you are done experimenting
